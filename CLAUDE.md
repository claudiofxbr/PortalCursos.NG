...
- JDK 21 / Node 24 (LTS) — Dependabot ignora o major do `typescript` (`.github/dependabot.yml`); TS 7 está bloqueado por falta de API programática que o `typescript-eslint` precisa. Rotina cloud diária ("PortalCursos.NG - TS7 unblock watcher", `trig_01Jwyio5CchKVmXnUwqqzDA5`, 09:00 UTC) checa `npm view typescript-eslint peerDependencies` no `frontend/`; enquanto o teto continuar abaixo de `7.0.0`, só loga "ainda bloqueado" sem alterar nada. Quando desbloquear, ela mesma faz o upgrade completo (typecheck+lint+build+testes), atualiza `dependabot.yml`/esta nota e abre um PR — mas **não** dá auto-merge (ajustado em 2026-09-13): o merge para `main` fica sempre pendente de revisão manual do usuário.
...
- **`@SQLDelete` em entidades com `@Version` — achado P0 corrigido em `Payment` (2026-09-13)**: `Payment` tinha `@SQLDelete(sql = "UPDATE payments SET active = false WHERE id = ?")` sem o parâmetro de `@Version` que o Hibernate injeta automaticamente para SQL customizado em entidade versionada — `paymentRepository.delete()`/`deleteById()` lançava `DataIntegrityViolationException` **sempre**, contra qualquer banco real (confirmado via Testcontainers). Como `PaymentService.deleteCharge` chamava `paymentRepository.delete()` diretamente, o endpoint `DELETE /api/finance/invoices/{id}` retornava 500 em produção, sem nenhum teste cobrindo isso. Corrigido removendo `@SQLDelete` de `Payment.java` e trocando `deleteCharge` para soft-delete explícito (`setActive(false) + save()`), mesmo padrão de `UserService.deleteUser`. `@Version` continua intacto (protege updates normais de concorrência).
  - **`Student` e `RepairTicket` (feito, 2026-09-13)**: mesmo padrão de `@SQLDelete` quebrado removido por higiene, já que era código morto (nenhum código de produção chamava `delete()`/`deleteById()` nessas entidades). Puro código removido, sem mudança de comportamento — soft-delete real continua sendo `setActive(false)+save()`. Não usar `delete()`/`deleteById()` do Spring Data em nenhuma entidade com `@Version` + soft-delete deste projeto.
  - `backend/src/test/java/com/portalcursos/ng02/PostgresMigrationIntegrationTest.java` tem o teste de regressão (`deleteChargeDesativaPagamentoSemErroContraPostgresReal`) — só um teste com banco real pega esse tipo de bug; testes mockados de `PaymentRepository` (`PaymentServiceTest`) não executam SQL de verdade.
