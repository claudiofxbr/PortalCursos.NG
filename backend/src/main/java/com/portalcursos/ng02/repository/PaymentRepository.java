...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    @Query("SELECT p FROM Payment p LEFT JOIN FETCH p.creator LEFT JOIN FETCH p.student s WHERE s.id = :studentId")
    ...
    @Query("SELECT p FROM Payment p LEFT JOIN FETCH p.creator LEFT JOIN FETCH p.student WHERE p.academicLevel = :academicLevel")
    List<Payment> findByAcademicLevel(@Param("academicLevel") EAcademicLevel academicLevel);

    @Query("SELECT p FROM Payment p LEFT JOIN FETCH p.creator LEFT JOIN FETCH p.student WHERE p.status IN :statuses")
    List<Payment> findByStatusIn(@Param("statuses") List<EPaymentStatus> statuses);

    @Query("SELECT p FROM Payment p LEFT JOIN FETCH p.creator LEFT JOIN FETCH p.student WHERE p.status = :status")
    List<Payment> findByStatus(@Param("status") EPaymentStatus status);

    @Query("SELECT p FROM Payment p LEFT JOIN FETCH p.creator LEFT JOIN FETCH p.student WHERE p.academicLevel = :level AND p.status IN :statuses")
    List<Payment> findByAcademicLevelAndStatusIn(@Param("level") EAcademicLevel level, @Param("statuses") List<EPaymentStatus> statuses);

    @Query("SELECT p FROM Payment p LEFT JOIN FETCH p.creator LEFT JOIN FETCH p.student WHERE p.academicLevel = :level AND p.status = :status")
    List<Payment> findByAcademicLevelAndStatus(@Param("level") EAcademicLevel level, @Param("status") EPaymentStatus status);

    @Query("SELECT p FROM Payment p LEFT JOIN FETCH p.creator LEFT JOIN FETCH p.student WHERE p.academicLevel = :level AND p.category = :category AND p.status IN :statuses")
